...
// DISPLAY TEMPLES — clears gallery and renders filtered list
...
function showHome()  { displayTemples(temples, 'Home'); }
function showOld()   { displayTemples(temples.filter(t => parseInt(t.dedicated) < 1900), 'Old'); }
function showNew()   { displayTemples(temples.filter(t => parseInt(t.dedicated) > 2000), 'New'); }
function showLarge() { displayTemples(temples.filter(t => t.area > 90000), 'Large'); }
function showSmall() { displayTemples(temples.filter(t => t.area < 10000), 'Small'); }

// NAV EVENT LISTENERS

document.getElementById('home').addEventListener('click',  e => { e.preventDefault(); showHome(); });
